...
public class ChestIntelligence extends FightIntelligence{
	...
	private Position getOptimizedMovement(TransformedCell[][] cells, MainEntity roxxor, Entity victim){
		...
		if(roxxor.getInfo().getStats().getMovementPoints() == 0){
			return null;
		}
		...
		if(positions.size() == 1 && UtilsMath.getPath(cells, roxxor.getPosition(), positions.get(0), false).size() == 2){
			return UtilsMath.getPath(cells, roxxor.getPosition(), positions.get(0), false).get(0);
		}
