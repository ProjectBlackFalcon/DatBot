...
public class ExchangeTypesItemsExchangerDescriptionForUserMessage extends NetworkMessage {
	...
	public void Serialize(DofusDataWriter writer) {
		try {
			writer.writeShort(this.itemTypeDescriptions.size());
			int _loc2_ = 0;
			while( _loc2_ < this.itemTypeDescriptions.size()){
				this.itemTypeDescriptions.get(_loc2_).Serialize(writer);
				_loc2_++;
			}
		} catch (Exception e){
			e.printStackTrace();
	...
	public void Deserialize(DofusDataReader reader) {
		try {
			...
			int _loc3_  = 0;
			this.itemTypeDescriptions = new ArrayList<BidExchangerObjectInfo>();
			while( _loc3_ <  _loc2_){
				BidExchangerObjectInfo _loc15_ = new BidExchangerObjectInfo();
				_loc15_.Deserialize(reader);
				this.itemTypeDescriptions.add(_loc15_);
				_loc3_++;
			}
		} catch (Exception e){
			e.printStackTrace();
	...
	//private void append(){
		//for(BidExchangerObjectInfo a : itemTypeDescriptions) {
			//Network.appendDebug("itemTypeDescriptions : " + a);
		//}
	//}
}
