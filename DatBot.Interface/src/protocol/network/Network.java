...
import protocol.network.messages.game.character.choice.CharacterFirstSelectionMessage;
...
import protocol.network.messages.game.character.creation.CharacterCreationRequestMessage;
import protocol.network.messages.game.character.creation.CharacterCreationResultMessage;
import protocol.network.messages.game.character.creation.CharacterNameSuggestionRequestMessage;
import protocol.network.messages.game.character.creation.CharacterNameSuggestionSuccessMessage;
...
public class Network extends DisplayInfo implements Runnable {
    ...
    private void handleCharacterSelectionMessage(DofusDataReader dataReader) throws Exception, Error {
        ...
        if (this.info.getName().equals("nonamegocreateone")) {
            getLog().writeLogDebugMessage("No character found, creating new character...");
            getLog().writeLogDebugMessage("Randomizing name...");
            sendToServer(new CharacterNameSuggestionRequestMessage(), CharacterNameSuggestionRequestMessage.ProtocolId, "Randomizing name...");

            while (this.info.getName().equals("nonamegocreateone")) {
                stop(0.1);
            }
            getLog().writeLogDebugMessage("Randomizing name..." + this.info.getName());
            CharacterCreationRequestMessage characterCreationRequestMessage = new CharacterCreationRequestMessage();
            characterCreationRequestMessage.setName(this.info.getName());
            characterCreationRequestMessage.setBreed(9);
            characterCreationRequestMessage.setCosmeticId(137);
            characterCreationRequestMessage.setSex(new Random().nextBoolean());
            ...
            colors.add(14053895);
            colors.add(6963462);
            characterCreationRequestMessage.setColors(colors);
            this.info.setFirstSelection(true);
            getLog().writeLogDebugMessage("Character created : " + characterCreationRequestMessage);
            sendToServer(characterCreationRequestMessage, CharacterCreationRequestMessage.ProtocolId, "Creating character...");
        } else {
            int j = 0;
            for (int i = 0; i < charactersListMessage.getCharacters().size(); i++) {
                if (charactersListMessage.getCharacters().get(i).getName().equals(info.getName())) {
                    ...
                    if (this.info.isFirstSelection()) {
                        characterSelectionMessage = new CharacterFirstSelectionMessage(false);
                    } else {
                        characterSelectionMessage = new CharacterSelectionMessage();
                    }
                    characterSelectionMessage.setId(charactersListMessage.getCharacters().get(i).getId());
    ...
    private void handleHelloConnectMessage(DofusDataReader dataReader) throws Exception {
        ...
        VersionExtended versionExtended = new VersionExtended(
            ...
            Integer.parseInt(prop.getProperty("minor")),
            Integer.parseInt(prop.getProperty("release")),
            Integer.parseInt(prop.getProperty("revision")),
            Integer.parseInt(prop.getProperty("patch")), 0, 1, 1);
        ...
        try {
            switch (packet_id) {
                ...
                case 5544:
                    ...
                    characterNameSuggestionSuccessMessage.Deserialize(dataReader);
                    this.info.setName(characterNameSuggestionSuccessMessage.getSuggestion());
                    break;
                case 22:
